...
def test_get_obs_counts_dataset():
    ...
    with tempfile.NamedTemporaryFile(suffix=".h5ad") as tmp:
        with h5py.File(tmp.name, "w") as f:
            obs = f.create_group("obs")
            obs.create_dataset("batch", data=np.array(["x", "y", "x", "z", "y", "y", "z"], dtype="S"))
        with h5py.File(tmp.name, "r") as f:
...
def test_obs_counts_json_string_dict(minimal_h5ad_file):
    with h5py.File(minimal_h5ad_file, "a") as f:
        ...
        f["obs"].create_dataset("batch", data=np.array(["x", "y", "x", "z", "y"], dtype="S"))

    info = extract.get_anndata_file_info(minimal_h5ad_file, obs_to_count=["cell_type", "batch"])
    obs_counts = info["obs_counts"]

    # Simulate the CLI/Parquet conversion
    obs_counts = {k: json.dumps(v) for k, v in obs_counts.items()}
